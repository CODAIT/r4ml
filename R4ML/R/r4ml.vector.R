...
setMethod("show", signature = "r4ml.vector", definition = function(object) {
  ...
  if (length(head.df) == 0) {
    colname <- SparkR::sparkR.callJMethod(object@jc, "toString")
    cat(paste0(colname, "\n"))
...
setMethod("ifelse", signature(test = "r4ml.vector", yes = "ANY", no = "ANY"),
  function(test, yes, no) {
    ...
    jc <- SparkR::sparkR.callJMethod(
            SparkR::sparkR.callJStatic("org.apache.spark.sql.functions", "when", test, yes),
            "otherwise", no
          )
