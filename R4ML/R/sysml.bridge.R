...
sysml.MLContext <- setRefClass("sysml.MLContext",
  ...
  methods = list(
    ...
    registerInput = function(dmlname, rdd_or_df, mc) {
      ...
      SparkR::sparkR.callJMethod(env$jref, "registerInput", dmlname, jrdd, mc$env$jref)
    },
    ...
    registerOutput = function(dmlname) {
      ...
      '
      stopifnot(class(dmlname) == "character")
      SparkR::sparkR.callJMethod(env$jref, "registerOutput", dmlname)
    },
...
sysml.MLOutput <- setRefClass("sysml.MLOutput",
  ...
  methods = list(
    ...
    getDF = function(colname, drop.id = TRUE) {
      ...
      df_jref <- SparkR::sparkR.callJMethod(env$jref, "getDF", env$sysmlSqlContext, colname)
...
\\cr
      ...
      vdf_jref <- SparkR::sparkR.callJStatic(env$jclass, fname, env$sparkContext, df@sdf)
    ...
    vectorDataFrameToBinaryBlock = function(df, mc, colname, id = FALSE) {
      ...
      vdf_jref <- SparkR::sparkR.callJStatic(env$jclass, fname, env$sparkContext, df@sdf, mc$env$jref, id, colname)
